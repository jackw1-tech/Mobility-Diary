...
di mobilità con Human Activity Recognition}
...
luoghi abituali. La posizione dell'utente è
...
\begin{itemize}
    ...
    verso uno storage a oggetti (MinIO) seguito da una fase di
    conferma esplicita, che evita che i dati grezzi transitino dal server
    ...
    modalità di mobilità (\texttt{IDLE}, \texttt{WALKING}, \texttt{RUNNING},
    \texttt{BIKING}, \texttt{MOVING\_VEHICLE}), con un'accuratezza dell'84,88\%
    sul validation set ufficiale;
    ...
    \item una dashboard Web per la consultazione del diario di mobilità
    da parte del personale autorizzato, con confronto diretto tra la
    traccia vera e la traccia perturbata secondo il livello di privacy
    selezionato.
\end{itemize}
...
Il sistema è composto da due client, un applicazione mobile,una
...
intervallo temporale, distanza e percorso \texttt{LineString}.
I \texttt{GpsPoint} mantengono timestamp, coordinate PostGIS, velocità e
...
\hline
\texttt{SignificantPlace} & \texttt{HabitualPlace}  \texttt{CandidateVisit} &
...
Completano lo stack PostgreSQL con PostGIS, Redis, i worker Celery.
...
un'architettura per progetti Flutter che separa in layer
distinti la UI, la logica di state management (Cubit/BLoC), i repository
...
la macchina a stati si trova in \texttt{movement} e deve stabilirbe se l'utente
...
non compresso, non supera la soglia di 12\,MB; a quel punto il buffer
...
circa 29\,000 byte. Un file da
12\,MB contiene quindi approssimativamente 430 finestre (circa 36
minuti di registrazione).
Questo valore è tenuto volutamente basso per limitare il picco di
...
richiesta, impedendo che due registrazioni si
...
geocoding Mapbox. La richiesta ~\cite{mapbox-directions} usa uno dei tre profili
...
sovrascrivere una scelta più recente. 
...
Il protocollo adottato usa il training set ufficiale,
...
L'ingresso è la finestra grezza
$500\times6$ di accelerometro e giroscopio.

I filtri convoluzionali apprendono pattern locali discriminanti e il
livello denso li comprime in un embedding di 128 valori.

Una GRU (Gated Recurrent Unit)~\cite{cho2014gru} è un tipo di rete
neurale ricorrente pensata per elaborare sequenze: a differenza della
CNN, che classifica ogni finestra di 5\,s isolatamente, la GRU
mantiene uno stato interno che si aggiorna finestra dopo finestra,
condizionando la predizione corrente sul contesto delle finestre
vicine. I suoi due gate (reset e update) regolano quanta informazione
dello stato precedente mantenere o scartare, permettendole di
catturare dipendenze anche su sequenze relativamente lunghe con meno
parametri di una LSTM equivalente.

Nel sistema la GRU è bidirezionale: legge la sequenza di embedding sia
in avanti sia all'indietro, in modo che la predizione su una finestra
sfrutti sia il contesto precedente sia quello successivo. Questo è il
motivo per cui è stata scelta al posto della sola CNN: una finestra
isolata non basta a distinguere, ad esempio, un veicolo fermo nel
traffico (circondato da altre finestre di guida) da una persona ferma
(circondata da cammino), un caso di ambiguità discusso nel dettaglio
in Sezione~\ref{sec:har-ambiguous}, mentre il contesto temporale
fornito dalla GRU rende questa distinzione possibile.
...
\texttt{geography} di PostGIS (SRID 4326). Questa
...
Le viste \emph{approximate} 
\emph{aggregated} sono ottenute con una tecnica di \emph{spatial cloaking}~\cite{gruteser2003cloaking}.
Ogni coordinata GPS viene proiettata in Web Mercator (EPSG:3857) e riposizionata al
...
distingue la linea sperimentale dal modello.
...
la Fig.~\ref{fig:har-seqlen-sweep} riportano l'andamento misurato
...
sul codice e sul modello reali (\texttt{HARClassifier}), nello stesso
processo, a caldo, per ciascuna lunghezza di sequenza e per dimensioni
di richiesta realistiche.
...
(Fig.~\ref{fig:isolated-label}), una finestra isolata non genera
...
La fusione del modello in un unico grafo e la chiamata in batch
riducono il costo di inferenza rispetto alla pipeline a due stadi
originale, senza cambiare i pesi né l'accuracy: il guadagno di
velocità deriva dal pattern di chiamata, quello di accuratezza dal
contesto più lungo dato alla GRU. Sono quindi due effetti distinti,
entrambi discussi con numeri nella Tabella~\ref{tab:har-variants} e
nella Tabella~\ref{tab:har-seqlen-latency}.
