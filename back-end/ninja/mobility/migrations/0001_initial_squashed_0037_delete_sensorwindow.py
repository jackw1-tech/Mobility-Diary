...
# Le funzioni RunPython di 0006_split_tripingestion_phase_status e
# 0026_remove_tripingestionpart_kind backfillavano/ripulivano righe di
# TripIngestion/TripIngestionPart preesistenti (modelli poi rinominati in
# TripUpload/TripUploadPart). Questa migration squashata viene eseguita solo
# su installazioni nuove, dove quelle righe non esistono: sostituite con
# no-op invece di riportare il codice originale.
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        # split_status backfillava righe TripIngestion preesistenti: su
        # un'installazione nuova (l'unico caso in cui questa migration
        # squashata viene eseguita) non ci sono righe da convertire, e il
        # modello e' comunque stato rinominato in TripUpload piu' avanti.
        ...
        # drop_legacy_non_raw_parts ripuliva righe preesistenti: su
        # un'installazione nuova non ce ne sono.
        migrations.RunPython(
            migrations.RunPython.noop,
            migrations.RunPython.noop,
        ),
        ...
        # normalize_expected_raw_parts normalizzava valori preesistenti:
        # su un'installazione nuova non ce ne sono.
        migrations.RunPython(
            migrations.RunPython.noop,
            migrations.RunPython.noop,
        ),
        ...
        # Le tre RenameIndex di 0033 sono gia' state assorbite: gli AddIndex
        # sopra sono stati ottimizzati per creare direttamente gli indici col
        # nome finale post-rename (683856/467800/54eab0), quindi i nomi
        # "vecchi" qui sotto non esisterebbero piu' in questo stato.
        migrations.AlterField(
            model_name='tripupload',
            name='source_trip',
