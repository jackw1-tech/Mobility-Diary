#!/usr/bin/env python3
"""
Fase B: temporal context via a GRU on top of the trained 6-channel CNN.

This script loads the chronologically ordered 9-channel dataset,
un-normalizes it on the fly, slices the first 6 channels (Acc + Gyr),
passes it through the 6-channel CNN to get embeddings, and then trains
a Bidirectional GRU on those embeddings to provide temporal smoothing.
"""
...
def embed_6ch(extractor: tf.keras.Model, X, batch_size: int, mean, std) -> np.ndarray:
    """Un-normalize, slice 6 channels, and extract embeddings in chunks."""
    N = X.shape[0]
    embs = []
    print(f"Extracting embeddings for {N} windows...")
    for i in range(0, N, batch_size):
        chunk = X[i : i + batch_size].copy()
        # Un-normalize (revert Z-score)
        chunk_unnorm = chunk * std + mean
        # Slice to 6 channels (Acc_x,y,z, Gyr_x,y,z)
        chunk_6ch = chunk_unnorm[:, :, :6]
        # Extract features
        e = extractor.predict(chunk_6ch, verbose=0)
        embs.append(e)
...
def parse_args():
    p = argparse.ArgumentParser(description="Train a GRU temporal head on 6-channel CNN embeddings.")
...
def main():
    ...
    norm_stats = json.loads((args.data_dir / f"{args.prefix}_norm_stats.json").read_text())
    ...
    print("Loading 6-channel CNN and building embedding extractor...")
    ...
    emb_batch = args.batch_size * 16  # Large batch since it's just inference
    emb_fit = embed_6ch(extractor, X_fit, emb_batch, mean, std)
    emb_es = embed_6ch(extractor, X_es, emb_batch, mean, std)
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(args.out_dir / "shl_6ch_5class_gru_best.keras",
    ...
    probs = model.predict(Xev, batch_size=args.batch_size, verbose=1)
    ...
    np.save(args.out_dir / "shl_6ch_5class_gru_val_pred.npy", y_pred)
    matrix = confusion_matrix(y_val, y_pred, num_classes)
    metrics = classification_metrics(matrix, CLASS_NAMES)
    print("\n=== HONEST metrics on official validation (6ch CNN + GRU) ===")
    print(json.dumps(metrics, indent=2))
    print("\nConfusion matrix:")
    print(matrix)

    (args.out_dir / "shl_6ch_5class_gru_val_metrics.json").write_text(json.dumps(metrics, indent=2))
    np.save(args.out_dir / "shl_6ch_5class_gru_confusion_matrix.npy", matrix)
