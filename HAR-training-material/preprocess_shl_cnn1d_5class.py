#!/usr/bin/env python3
"""
Memory-conscious SHL/HAR preprocessing for a 1D CNN (5-class variant).

Identical to preprocess_shl_cnn1d.py, but DRIVING and PUBLIC_TRANSPORT are
merged into a single MOVING_VEHICLE class. This removes the hard DRIVING vs
PUBLIC_TRANSPORT boundary while keeping RUNNING (the most distinctive class)
separate.
...
def remap_labels(raw_labels: np.ndarray) -> np.ndarray:
    """
    Map SHL labels (5-class variant):
      1 -> 0 IDLE
      2 -> 1 WALKING
      3 -> 2 RUNNING
      4 -> 3 BIKING
      5 -> 4 MOVING_VEHICLE        (was DRIVING)
      6,7,8 -> 4 MOVING_VEHICLE    (was PUBLIC_TRANSPORT: Bus/Train/Subway)
    ...
    y[raw_labels == 3] = 2
    y[raw_labels == 4] = 3
    y[np.isin(raw_labels, (5, 6, 7, 8))] = 4
    return y
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a sampled SHL/HAR tensor X=(N, 500, 6) and remapped 5-class labels y."
    )
    parser.add_argument("--dataset-root", type=Path, default=Path("DATASET"))
    parser.add_argument("--out-dir", type=Path, default=Path("processed_5class"))
    parser.add_argument("--output-prefix", default="shl_cnn1d_full_100pct_5class")
