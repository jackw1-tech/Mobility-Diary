...
const owner = computed(() => dashboards.value[0]?.owner ?? null);
...
async function loadDailyDashboard() {
  if (!userId.value || !day.value) return;
  loading.value = true;
  error.value = '';
  dashboards.value = [];
  try {
    const response = await fetchUserTrips(userId.value);
    const dayTrips = response.trips.filter((trip) => tripOverlapsDay(
      trip.started_at,
      trip.ended_at,
    ));
    dashboards.value = await Promise.all(
      dayTrips.map((trip) => fetchTripDashboard(userId.value, trip.id)),
    );
  } catch (unknownError) {
    dashboards.value = [];
...
async function changeDay(nextDay: string) {
  ...
  await router.push({ name: 'daily-dashboard', params: { userId: userId.value, day: nextDay } });
}
...
<template>
  <section class="page-stack" aria-labelledby="daily-dashboard-title">
    ...
    <div class="section-heading">
      <div>
        <p class="eyebrow">Dashboard giornaliera</p>
