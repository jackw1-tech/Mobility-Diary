#!/usr/bin/env bash
# Crea la parent issue e le 5 issue verticali per "Core Ingestion inline".
# Prerequisiti:
#   1) gh installato
#   2) gh auth login con accesso a jackw1-tech/Mobility-Diary
# Esecuzione:
#   bash docs/create_issues_core_ingestion_inline.sh
set -euo pipefail
...
PARENT_URL="$(gh issue create --repo "$REPO" \
  --title "PRD: Core Ingestion inline sincrona" \
...
ISSUE_1_URL="$(gh issue create --repo "$REPO" \
  --title "State model: distinguere fase core/raw da modalita' di caricamento" \
  --label feature --label backend --label mobile --label ready-for-agent \
  --body "$(cat <<EOF
## Parent
...
Rendere esplicito nel sistema che \`core_status\` e \`raw_status\` descrivono la fase dell'Ingestione del Viaggio, mentre una nuova modalita' descrive come il Core e' stato ricevuto: vecchio caricamento a parti oppure Core Ingestion inline.

Questa slice non deve ancora cambiare il percorso mobile principale. Deve solo preparare schema, stati osservabili e test, cosi' le slice successive possano implementare la POST core senza ambiguita'.
...
ISSUE_2_URL="$(gh issue create --repo "$REPO" \
  --title "Backend: Core Ingestion inline completa via API" \
...
Implementare il percorso backend primario \`POST /api/ingestion/trips/core\`. L'endpoint riceve metadata, GPS points, state transitions, \`expected_raw_parts\` e \`core_payload_sha256\`, poi materializza sincronicamente il Core dentro la request HTTP.

Il risultato verificabile di questa slice e' che un client autenticato possa sincronizzare un Viaggio piccolo in una sola request e ottenere subito \`trip_id\`, conteggi, distanza e \`map_available\`, senza object storage, Celery o polling nel path felice.
...
ISSUE_3_URL="$(gh issue create --repo "$REPO" \
  --title "Mobile: SyncJob usa POST core inline" \
...
Cambiare il percorso mobile nuovo in modo che il Core non venga piu' spedito come \`gps_points.json.gz\` e \`state_transitions.json.gz\` via presigned upload. Il SyncJob deve costruire un JSON inline deterministico, calcolare \`corePayloadSha256\`, chiamare \`POST /api/ingestion/trips/core\`, salvare \`remoteIngestionId\`, \`remoteTripId\` e \`coreMapAvailable\`, poi aggiornare lo stato locale del core a completed.
...
ISSUE_4_URL="$(gh issue create --repo "$REPO" \
  --title "Raw Sensor Ingestion continua dopo Core Inline" \
  --label feature --label backend --label mobile --label ready-for-agent \
  --body "$(cat <<EOF
## Parent
...
Completare il comportamento "core subito, raw dopo". Dopo la risposta positiva della POST core inline, lo stesso SyncJob deve continuare a caricare i raw \`sensor_windows\` con il flusso presigned esistente, usando l'ingestion id ritornato dalla POST core.

La riuscita o il fallimento retryable del raw non deve tornare a bloccare il Viaggio Sincronizzato o la Traiettoria del Viaggio gia' resa disponibile dal core.
...
ISSUE_5_URL="$(gh issue create --repo "$REPO" \
  --title "Legacy compatibility e hardening operativo per Core Ingestion" \
  --label chore --label backend --label mobile --label ready-for-agent \
  --body "$(cat <<EOF
## Parent
...
Rafforzare il bordo tra nuovo Core Ingestion inline e vecchio Core Ingestion a parti. Gli endpoint legacy devono restare funzionanti per client vecchi, ma il mobile nuovo non deve usarli per GPS/state. La documentazione deve rendere chiaro quali stati appartengono al path inline, quali al path legacy e quali al raw.
